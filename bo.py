...
def command_start(update, context):
    update.message.reply_text('Привет! В меня можно ввести слово по-русски, а я выведу его наиболее вероятные переводы и контексты, в которых оно встречается, на финском!')
...
def handler_echo(update, context):
    ...
    context_s = contexting(corpus_maker(), request)
    top5words = improver(context_s, request)
    if len(context_s) > 0:
        if request in badwords:
            if isinstance(towrite, list):
                ...
                update.message.reply_text(badwords.get(request))
                update.message.reply_text(towrite)
        else:
            if isinstance(towrite, list):
                towrite = ''.join(towrite)
                update.message.reply_text(request)
                update.message.reply_text(top5words)
                update.message.reply_text(towrite)
    else:
        update.message.reply_text('Пушкин такими словами не кидался..')
